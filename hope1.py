...
#		Ab.forward()
	...
	print(ROI_image)
	line_arr = hough_lines(ROI_image, 1, 1*np.pi/180,30,10,20)
	print(line_arr)
	...
	slope_degree = slop_degree[np.abs(slope_degree)>95]

	L_lines, R_lines = line_arr[(slope_degree>0),:], line_arr[(slope_degree<0):]
	temp = np.zeros((image.shape[0], image.shape[1],3), dtype = np.unit8)
	...
	draw_fitline(temp, right_fit_line)

	result = weighted_image(temp, image)
	cv2.imshow('r',result)
	cv2.waitKey(0)
